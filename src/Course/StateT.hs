...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f (StateT fu) = StateT (\s -> (\(ai, si) -> (f ai, si)) <$> fu s)
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) faf (StateT sa) = StateT (\s ->
    do
      (a, s1) <- sa s
      let StateT sfb = faf a
      sfb s1
    )
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' (StateT sf) = runExactlyOne . sf
...
distinct' ::
  ...
  List a
  -> List a
distinct' x = eval' (filtering (\a -> state' (\set -> (S.notMember a set, S.insert a set))) x) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF l = evalT (filtering (
    \a -> StateT (
      \s -> if a > 100 then Empty else Full (S.notMember a s, S.insert a s))
  ) l) S.empty
...
instance Monad f => Applicative (OptionalT f) where
  ...
  (<*>) ::
    ...
    -> OptionalT f a
    -> OptionalT f b
  (<*>) (OptionalT fab) (OptionalT fa) = OptionalT (
    fab >>= onFull (\ab -> (ab <$>) <$> fa))
...
instance Monad f => Monad (OptionalT f) where
  (=<<) ::
    ...
    -> OptionalT f a
    -> OptionalT f b
  (=<<) aofb (OptionalT fa) = OptionalT (onFull (runOptionalT . aofb) =<< fa)
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  (=<<) f (Logger l a) = (\(Logger li b) -> Logger (l ++ li) b) $ f a
  ...
  runOptionalT (evalT (filtering (\a -> StateT(\s -> 
    OptionalT(if a > 100 
      then log1 (fromString ("aborting > 100: " P.++ show a)) Empty
        ...
        else pure) (Full (S.notMember a s, S.insert a s))))) l) S.empty)
