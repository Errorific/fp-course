...
hex = thisMany 4 (satisfy isHexDigit)
  >>= (\a ->
          case readHex a of
            (Full c) -> valueParser $ chr c
            Empty    -> constantParser $ UnexpectedString a
      )
...
eof ::
  Parser ()
eof = P (\i -> if isEmpty i then Result i () else UnexpectedString i)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lp = satisfy $ and . sequence lp
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lp = satisfy $ or . sequence lp
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma s e p = betweenCharTok s e $ sepby p commaTok
